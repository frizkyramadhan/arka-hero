...
class BulkLeaveRequestController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('leave-requests.bulk-index', compact('batches'))
            ->with('title', 'Periodic Leave Requests');
    }
        ...
        try {
            ...
            $formattedData = $employeesDue->map(function ($item) {
                ...
                return [
                    ...
                    'department_id' => $department && $department->id ? $department->id : null,
    ...
    /**
     * Get approver selector component for a department (AJAX)
     */
    public function getApproverSelector(Request $request)
    {
        ...
        return response()->view('components.bulk-approver-selector', [
    ...
    public function getBulkApprovalPreview(Request $request)
    {
        try {
            ...
            foreach ($administrations as $admin) {
                ...
                $departmentGroups[$deptId]['employee_count']++;

                // Add employee info (avoid duplicates)
                $employeeExists = false;
                foreach ($departmentGroups[$deptId]['employees'] as $existingEmp) {
                    if ($existingEmp['name'] === ($admin->employee->fullname ?? 'Unknown')) {
                        $employeeExists = true;
                        break;
                    }
                }
    ...
    {
        // Parse department_approvers from JSON strings to arrays before validation
        $departmentApprovers = $request->get('department_approvers', []);
        $parsedDepartmentApprovers = [];

        foreach ($departmentApprovers as $deptId => $approvers) {
            if (is_string($approvers)) {
                ...
                if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                    $parsedDepartmentApprovers[$deptId] = $decoded;
                } else {
                    // If not valid JSON, treat as empty array
                    $parsedDepartmentApprovers[$deptId] = [];
                }
            } elseif (is_array($approvers)) {
                // Already an array, use as is
                $parsedDepartmentApprovers[$deptId] = $approvers;
            } elseif (is_null($approvers)) {
                // Null value, set as empty array
                $parsedDepartmentApprovers[$deptId] = [];
            } else {
                // Invalid format, set as empty array
                $parsedDepartmentApprovers[$deptId] = [];
            }
        }

        // Merge parsed department approvers back to request
        $request->merge(['department_approvers' => $parsedDepartmentApprovers]);
        ...
        try {
            ...
            // Get department approvers mapping
            $departmentApprovers = $request->get('department_approvers', []);

            // Build a map of administration_id -> department_id for quick lookup
            $administrationDepartmentMap = [];
            foreach ($request->selected_employees as $empData) {
                $administration = Administration::find($empData['administration_id']);
                if ($administration && $administration->position) {
                    $administrationDepartmentMap[$administration->id] = $administration->position->department_id;
                }
            }
            ...
            foreach ($request->selected_employees as $empData) {
                try {
                    ...
                    // Get department_id for this employee
                    $departmentId = $administration->position->department_id ?? null;

                    // Get approvers for this department (already parsed as arrays)
                    $approverIds = [];
                    if ($departmentId && isset($departmentApprovers[$departmentId])) {
                        // Already parsed as array in the beginning
                        $approverIds = is_array($departmentApprovers[$departmentId])
                            ? $departmentApprovers[$departmentId]
                            : [];
                    } else {
                        // Fallback: try to find by department name (for cases where department_id is null)
                        $departmentName = $administration->position->department->department_name ?? null;
                        if ($departmentName) {
                            foreach ($departmentApprovers as $key => $approvers) {
                                // Check if key contains department name or is a string match
                                if (is_string($key) && strpos($key, $departmentName) !== false) {
                                    $approverIds = is_array($approvers) ? $approvers : [];
                                    break;
                                }
                            }
                        }
                    ...
                    // Create leave request with manual approvers
            ...
            return redirect()->route('leave.periodic-requests.show', ['batch_id' => $batchId])
    ...
    public function show($batchId)
    {
        $leaveRequests = LeaveRequest::where('batch_id', $batchId)
            ...
            ->select('leave_requests.*', 'departments.department_name', 'administrations.nik')
        ...
        return view('leave-requests.bulk-show', compact('leaveRequests', 'batchInfo', 'statusCounts'))
            ->with('title', 'Periodic Leave Request Detail');
    }
