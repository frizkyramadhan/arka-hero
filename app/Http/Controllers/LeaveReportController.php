...
class LeaveReportController extends Controller
{
    ...
    /**
     * Map paginated entitlements to rows for the detailed entitlement report view.
     */
    private function transformEntitlementPaginatorForDetailedReport(LengthAwarePaginator $paginator): LengthAwarePaginator
    {
        $paginator->setCollection(
            $paginator->getCollection()->map(function (LeaveEntitlement $entitlement) {
                return array_merge($entitlement->getLeaveCalculationDetails(), [
    ...
    public function entitlementDetailed(Request $request)
    {
        ...
        if ($request->filled('year') || $request->filled('employee_id') || $request->filled('leave_type_id') || $request->has('show_all')) {
            ...
            $entitlements = $this->transformEntitlementPaginatorForDetailedReport($query->paginate(50));
        }
        ...
        if ($request->filled('conversion_status') || $request->filled('employee_id') || $request->has('show_all')) {
            ...
            $autoConversions->setCollection(
                $autoConversions->getCollection()->map(function ($leaveRequest) {
                    $daysUntilConversion = now()->diffInDays($leaveRequest->auto_conversion_at, false);
                    ...
                    return [
                        ...
                        'start_date' => $leaveRequest->displayStartDate('d M Y'),
                        'end_date' => $leaveRequest->displayEndDate('d M Y'),
                        'total_days' => $leaveRequest->displayTotalDaysLabel(),
                        'is_cashout_only' => $leaveRequest->isLSLCashoutOnly(),
                        'auto_conversion_at' => $leaveRequest->auto_conversion_at->format('d M Y H:i'),
    ...
    public function exportMonitoring(Request $request)
    {
        ...
        $data = $query->get()->map(function (LeaveRequest $leaveRequest) {
            return [
                ...
                'Start Date' => $leaveRequest->displayStartDate('d M Y'),
                'End Date' => $leaveRequest->displayEndDate('d M Y'),
                'Total Days' => $leaveRequest->displayTotalDaysLabel(),
    ...
    public function exportCancellation(Request $request)
    {
        ...
        $data = $query->get()->map(function ($cancellation) {
            return [
                ...
                'Original Start Date' => $cancellation->leaveRequest->displayStartDate('d M Y'),
                'Original End Date' => $cancellation->leaveRequest->displayEndDate('d M Y'),
                'Original Total Days' => $cancellation->leaveRequest->displayTotalDaysLabel(),
