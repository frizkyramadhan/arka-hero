...
class ApprovalRequestController extends Controller
{
    ...
    /**
     * API: Get user's pending approvals
     */
    public function apiMyApprovals()
    {
        // Note: Removed authorization check as per user's request
        $user = Auth::user();

        $approvals = ApprovalPlan::with([
            'approver',
            'officialtravel.creator',
            'recruitmentRequest.createdBy',
            'recruitmentRequest.position',
            ...
            ->map(function ($approval) {
                ...
                if ($approval->leaveRequest) {
                    $document = $approval->leaveRequest;
                    $documentType = 'Leave Request';
                    $title = ($document->leaveType->name ?? 'N/A') . ' - ' . ($document->administration->employee->fullname ?? 'N/A');
                    $description = date('M d', strtotime($document->start_date)) . ' - ' . date('M d, Y', strtotime($document->end_date)) . ' (' . $document->total_days . ' days)';
                    $url = route('leave.requests.show', $document->id);
                } elseif ($approval->officialtravel) {
                    $document = $approval->officialtravel;
                    $documentType = 'Official Travel';
                    $title = $document->destination;
                    $description = 'Travel date: ' . date('M d, Y', strtotime($document->official_travel_date));
                    $url = route('officialtravels.show', $document->id);
                } elseif ($approval->recruitmentRequest) {
                    $document = $approval->recruitmentRequest;
                    $documentType = 'Recruitment Request';
                    $title = ($document->position->position_name ?? 'N/A') . ' - ' . ($document->project->project_code ?? 'N/A');
                    $description = 'Required: ' . $document->required_quantity . ' person(s)';
                    $url = route('recruitment.requests.show', $document->id);
                }
                ...
                return [
                    ...
                    'url' => $url,
                    'approve_url' => route('approval.process', $approval->id) . '?action=approve',
                    'reject_url' => route('approval.process', $approval->id) . '?action=reject',
                ];
            });
