    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            foreach ($rows as $rowIndex => $row) {
                ...
                // If ID is provided, validate that the job experience record exists
                if (!empty($row['id'])) {
                    $existingJobExp = Jobexperience::where('id', $row['id'])->first();

                    if (!$existingJobExp) {
                        $validator->errors()->add(
                            $rowIndex . '.id',
                            "Job experience record with ID {$row['id']} not found"
                        );
                    }
                }
            }
    ...
    public function model(array $row)
    {
        ...
        try {
            ...
            $message = strpos($e->getMessage(), 'Duplicate entry') !== false
                ? "Duplicate job experience record found. Please check if this job experience record already exists."
                : 'Database error: ' . $e->getMessage();

            $attribute = strpos($e->getMessage(), 'Duplicate entry') !== false ? 'company_name' : 'system_error';
            $value = strpos($e->getMessage(), 'Duplicate entry') !== false ? $row['company_name'] : null;
