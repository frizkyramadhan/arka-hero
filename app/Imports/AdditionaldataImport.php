...
class AdditionaldataImport implements ToModel, WithHeadingRow, WithValidation, SkipsOnError, SkipsOnFailure, WithEvents, WithChunkReading, WithBatchInserts
{
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            foreach ($rows as $rowIndex => $row) {
                ...
                // If ID is provided, validate that the additional data record exists
                if (!empty($row['id'])) {
                    $existingData = Additionaldata::where('id', $row['id'])->first();

                    if (!$existingData) {
                        $validator->errors()->add(
                            $rowIndex . '.id',
                            "Additional data record with ID {$row['id']} not found"
                        );
                    }
                }
            }
    ...
    public function model(array $row)
    {
        ...
        try {
            ...
            $additionalData = Additionaldata::updateOrCreate(
                [
                    'employee_id' => $employee->id,
                ],
                $additionalData
            ...
            $message = strpos($e->getMessage(), 'Duplicate entry') !== false
                ? "Duplicate additional data record found. Please check if this record already exists."
                : 'Database error: ' . $e->getMessage();

            $attribute = strpos($e->getMessage(), 'Duplicate entry') !== false ? 'employee_id' : 'system_error';
            $value = strpos($e->getMessage(), 'Duplicate entry') !== false ? $employee->id : null;
