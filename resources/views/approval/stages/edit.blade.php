...
@section('content')
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Edit Approval Stage</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">Edit {{ $stage->stage_name }}</li>
                    </ol>
                </div>
            </div>
        </div>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">Edit Stage: {{ $stage->stage_name }}</h3>
                            ...
                            <form id="stageForm">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" id="stage_name" name="stage_name"
                                                value="{{ $stage->stage_name }}" required>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            ...
                                            <input type="number" class="form-control" id="stage_order" name="stage_order"
                                                value="{{ $stage->stage_order }}" min="1" required>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" id="stage_type" name="stage_type" required>
                                                <option value="sequential"
                                                    {{ $stage->stage_type === 'sequential' ? 'selected' : '' }}>Sequential
                                                </option>
                                                <option value="parallel"
                                                    {{ $stage->stage_type === 'parallel' ? 'selected' : '' }}>Parallel
                                                </option>
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="number" class="form-control" id="escalation_hours"
                                                name="escalation_hours" value="{{ $stage->escalation_hours }}"
                                                min="1">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <div id="approvers-container">
                                        <!-- Existing approvers will be loaded here -->
                                    </div>
...
@section('scripts')
    <script>
        ...
        const existingStage = @json($stage);
        ...
        function loadExistingApprovers() {
            if (!existingStage.approvers || existingStage.approvers.length === 0) {
                addApprover();
                return;
            }

            existingStage.approvers.forEach(approver => {
                addApprover();
                const lastApprover = document.querySelector('.approver-item:last-child');

                // Set approver values
                lastApprover.querySelector('.approver-type').value = approver.approver_type;
                updateApproverOptions(lastApprover.querySelector('.approver-type'));

                setTimeout(() => {
                    lastApprover.querySelector('.approver-id').value = approver.approver_id;
                    lastApprover.querySelector('[name*="is_backup"]').checked = approver.is_backup;
                }, 100);
            });
        }
        ...
        document.getElementById('stageForm').addEventListener('submit', function(e) {
            ...
            // Submit via AJAX
            fetch('{{ route('approval.stages.update', [$flow, $stage]) }}', {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    if (data.success) {
                        showAlert('success', 'Stage updated successfully!');
                        setTimeout(() => {
                            window.location.href = '{{ route('approval.stages.index', $flow) }}';
                        }, 1500);
                    } else {
                        showAlert('error', data.message || 'Failed to update stage');
                        if (data.errors) {
                ...
                .catch(error => {
                    ...
                    showAlert('error', 'Failed to update stage');
                });
        });
        ...
        // Initialize with existing approvers
