    ...
    <div class="content-wrapper-custom">
        ...
        <div class="fptk-header">
            <div class="fptk-header-content">
                ...
                <div class="fptk-status-pill status-pending-approval">
        ...
        <div class="fptk-content">
            <div class="row">
                ...
                <div class="col-lg-8">
                    ...
                    <!-- Approval Status -->
                    <div class="fptk-card approval-status-card">
                        <div class="card-head">
                            <h2><i class="fas fa-user-check"></i> Approval Status</h2>
                        </div>
                        <div class="card-body">
                            <div class="status-grid">
                                <!-- HR Acknowledgment Status -->
                                <div class="status-item">
                                    <div class="status-icon" style="background-color: #f39c12;">
                                        <i class="fas fa-user-check"></i>
                                    </div>
                                    <div class="status-content">
                                        <div class="status-label">HR Acknowledger</div>
                                        <div class="status-value">{{ $fptk->acknowledger->name ?? 'N/A' }}</div>
                                        <div class="status-meta">
                                            <span
                                                class="badge badge-{{ $fptk->known_status == 'approved' ? 'success' : ($fptk->known_status == 'rejected' ? 'danger' : 'warning') }}">
                                                {{ ucfirst($fptk->known_status) }}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <!-- PM Approval Status -->
                                <div class="status-item">
                                    <div class="status-icon" style="background-color: #e67e22;">
                                        <i class="fas fa-user-shield"></i>
                                    </div>
                                    <div class="status-content">
                                        <div class="status-label">Project Manager</div>
                                        <div class="status-value">{{ $fptk->projectManagerApprover->name ?? 'N/A' }}</div>
                                        <div class="status-meta">
                                            <span
                                                class="badge badge-{{ $fptk->pm_approval_status == 'approved' ? 'success' : ($fptk->pm_approval_status == 'rejected' ? 'danger' : 'warning') }}">
                                                {{ ucfirst($fptk->pm_approval_status) }}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                @if ($fptk->known_at)
                                    <div class="status-item">
                                        ...
                                        <div class="status-content">
                                            <div class="status-label">HR Acknowledgment Date</div>
                                ...
                                @if ($fptk->pm_approved_at)
                                    <div class="status-item">
                                        <div class="status-icon" style="background-color: #9b59b6;">
                                            <i class="fas fa-calendar-alt"></i>
                                        </div>
                                        <div class="status-content">
                                            <div class="status-label">PM Approval Date</div>
                                            <div class="status-value">
                            ...
                            @if ($fptk->known_remark)
                                <div class="remark-section mt-4">
                                    <h3><i class="fas fa-comment"></i> HR Acknowledgment Notes</h3>
                            ...
                            @if ($fptk->pm_approval_remark)
                                <div class="remark-section mt-4">
                                    <h3><i class="fas fa-comment"></i> PM Approval Notes</h3>
                                    <div class="remark-content">
                ...
                <div class="col-lg-4">
                    <div class="fptk-card approval-card">
                        <div class="card-head">
                            <h2><i class="fas fa-crown"></i> Director Approval Decision</h2>
                        </div>
                        <div class="card-body">
                            <!-- Decision Buttons -->
                            ...
                            <!-- Director Approval Form -->
                            <form action="{{ route('recruitment.requests.approve-director', $fptk->id) }}" method="POST"
                                id="directorApprovalForm">
                                @csrf
                                <input type="hidden" name="director_approval_status" id="director_approval_status"
                                    value="">

                                <div class="form-group">
                                    <label for="director_approval_remark">Director Approval Notes <span
                                            class="text-danger">*</span></label>
                                    <textarea class="form-control @error('director_approval_remark') is-invalid @enderror" name="director_approval_remark"
                                        id="director_approval_remark" rows="4"
                                        placeholder="Please provide your approval details:
• Decision rationale
• Any conditions or requirements
• Additional notes or observations"
                                        required>{{ old('director_approval_remark') }}</textarea>
                                    @error('director_approval_remark')
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const statusInput = document.getElementById('director_approval_status');
            const submitBtn = document.querySelector('.submit-btn');
            const remarkTextarea = document.getElementById('director_approval_remark');
            ...
            document.getElementById('directorApprovalForm').addEventListener('submit', function(e) {
                ...
                if (!remarkTextarea.value.trim()) {
                    e.preventDefault();
                    alert('Please provide director approval notes.');
                    return;
                }

                const status = statusInput.value;
                const confirmMessage = status === 'approved' ?
                    'Are you sure you want to approve this FPTK as Director? This is the final approval.' :
                    'Are you sure you want to reject this FPTK as Director? This will deny the FPTK.';
