...
@section('content')
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item"><a
                                href="{{ route('recruitment.mpp.show', $mpp->id) }}">{{ $mpp->mpp_number }}</a></li>
    ...
    <section class="content">
        <div class="container-fluid">
            <form id="mpp-form" action="{{ route('recruitment.mpp.update', $mpp->id) }}" method="POST">
                @csrf
                @method('PUT')
                            ...
                            <div class="card-body">
                                            ...
                                            <select name="project_id" id="project_id"
                                                ...
                                                @foreach ($projects as $project)
                                                    <option value="{{ $project->id }}"
                                                        {{ old('project_id', $mpp->project_id) == $project->id ? 'selected' : '' }}>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="text" name="title" id="title"
                                                    ...
                                                    value="{{ old('title', $mpp->title) }}" required
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror"
                                        rows="3" placeholder="Additional information about this MPP (optional)">{{ old('description', $mpp->description) }}</textarea>
                                    @error('description')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                                ...
                                                <tr class="detail-row">
                                                    ...
                                                    <td>
                                                        <input type="hidden" name="details[{{ $index }}][id]"
                                                            value="{{ $detail->id }}">
                                                        <select name="details[{{ $index }}][position_id]"
                                                            class="form-control form-control-sm select2-position" required
                                                            style="width: 100%;">
                                                            <option value="">Select Position</option>
                                                            @foreach ($positions as $position)
                                                                <option value="{{ $position->id }}"
                                                                    {{ old('details.' . $index . '.position_id', $detail->position_id) == $position->id ? 'selected' : '' }}>
                                                    ...
                                                    <td>
                                                        <input type="number"
                                                            name="details[{{ $index }}][qty_unit]"
                                                            class="form-control form-control-sm text-center qty-unit"
                                                            value="{{ old('details.' . $index . '.qty_unit', $detail->qty_unit) }}"
                                                            min="0" placeholder="0">
                                                    </td>
                                                    <td class="plan-cell">
                                                        <input type="number"
                                                            name="details[{{ $index }}][plan_qty_s]"
                                                            class="form-control form-control-sm text-center plan-s"
                                                            value="{{ old('details.' . $index . '.plan_qty_s', $detail->plan_qty_s) }}"
                                                            min="0" required>
                                                    </td>
                                                    <td class="plan-cell">
                                                        <input type="number"
                                                            name="details[{{ $index }}][plan_qty_ns]"
                                                            class="form-control form-control-sm text-center plan-ns"
                                                            value="{{ old('details.' . $index . '.plan_qty_ns', $detail->plan_qty_ns) }}"
                                                            min="0" required>
                                                    </td>
                                                    <td class="text-center plan-cell">
                                                        <span
                                                            class="badge badge-info plan-total">{{ $detail->total_plan }}</span>
                                                    </td>
                                                    <td class="existing-cell">
                                                        <input type="number"
                                                            name="details[{{ $index }}][existing_qty_s]"
                                                            class="form-control form-control-sm text-center existing-s"
                                                            value="{{ old('details.' . $index . '.existing_qty_s', $detail->existing_qty_s) }}"
                                                            min="0" required>
                                                    </td>
                                                    <td class="existing-cell">
                                                        <input type="number"
                                                            name="details[{{ $index }}][existing_qty_ns]"
                                                            class="form-control form-control-sm text-center existing-ns"
                                                            value="{{ old('details.' . $index . '.existing_qty_ns', $detail->existing_qty_ns) }}"
                                                            min="0" required>
                                                    </td>
                                                    <td class="text-center existing-cell">
                                                        <span
                                                            class="badge badge-warning existing-total">{{ $detail->total_existing }}</span>
                                                    </td>
                                                    <td class="text-center diff-cell">
                                                        <span
                                                            class="badge diff-s {{ $detail->diff_s > 0 ? 'diff-positive' : ($detail->diff_s < 0 ? 'diff-negative' : 'diff-zero') }}">{{ $detail->diff_s > 0 ? '+' : '' }}{{ $detail->diff_s }}</span>
                                                    </td>
                                                    <td class="text-center diff-cell">
                                                        <span
                                                            class="badge diff-ns {{ $detail->diff_ns > 0 ? 'diff-positive' : ($detail->diff_ns < 0 ? 'diff-negative' : 'diff-zero') }}">{{ $detail->diff_ns > 0 ? '+' : '' }}{{ $detail->diff_ns }}</span>
                                                    </td>
                                                    <td class="text-center diff-cell">
                                                        <span
                                                            class="badge diff-total {{ $detail->total_diff > 0 ? 'diff-positive' : ($detail->total_diff < 0 ? 'diff-negative' : 'diff-zero') }}">{{ $detail->total_diff > 0 ? '+' : '' }}{{ $detail->total_diff }}</span>
                                                    </td>
                                                    <td class="text-center">
                                                        <div class="custom-control custom-checkbox">
                                                            <input type="checkbox" class="custom-control-input"
                                                                id="requires_theory_test_{{ $index }}"
                                                                name="details[{{ $index }}][requires_theory_test]"
                                                                value="1"
                                                                {{ old('details.' . $index . '.requires_theory_test', $detail->requires_theory_test) ? 'checked' : '' }}>
                                                            <label class="custom-control-label"
                                                                for="requires_theory_test_{{ $index }}">
                                                            </label>
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <select name="details[{{ $index }}][agreement_type]"
                                                            class="form-control form-control-sm" required>
                                                            <option value="pkwt"
                                                                {{ old('details.' . $index . '.agreement_type', $detail->agreement_type ?? 'pkwt') == 'pkwt' ? 'selected' : '' }}>
                                                                PKWT</option>
                                                            <option value="pkwtt"
                                                                {{ old('details.' . $index . '.agreement_type', $detail->agreement_type ?? 'pkwt') == 'pkwtt' ? 'selected' : '' }}>
                                                                PKWTT</option>
                                                            <option value="magang"
                                                                {{ old('details.' . $index . '.agreement_type', $detail->agreement_type ?? 'pkwt') == 'magang' ? 'selected' : '' }}>
                                                                Magang</option>
                                                            <option value="harian"
                                                                {{ old('details.' . $index . '.agreement_type', $detail->agreement_type ?? 'pkwt') == 'harian' ? 'selected' : '' }}>
                                                                Harian</option>
                                                        </select>
                                                    </td>
                                                    <td class="text-center">
                                                        ...
                                                        @else
                                                            <span class="badge badge-info"
                                                                title="Has sessions, cannot delete">
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-2 mb-md-0">
                                        <button type="submit" class="btn btn-primary btn-block">
                                            <i class="fas fa-save mr-2"></i> Update MPP
                                        </button>
                                    </div>
                                    <div class="col-md-6">
                                        <a href="{{ route('recruitment.mpp.show', $mpp->id) }}"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let rowIndex = {{ $mpp->details->count() - 1 }};
        ...
        `;
            ...
            function updateRowNumbers() {
                $('.detail-row').each(function(index) {
                    ...
                    // Update input names (including hidden id input)
