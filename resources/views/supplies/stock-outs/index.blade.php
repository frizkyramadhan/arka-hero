                    ...
                    <div class="card-header">
                        ...
                        <div class="card-tools">
                            @can('supplies.stock-out.show')
                                <a href="{{ route('supplies.stock-outs.export') }}" id="btn-export-stock-out"
                                    class="btn btn-success">
                                    <i class="fas fa-download"></i> Export
                                </a>
                            @endcan
                            @canany(['supplies.stock-out.create', 'supplies.stock-out.edit'])
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="importModalLabel">Import Stock Out</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                            <form action="{{ route('supplies.stock-outs.import') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="modal-body">
                                    <div class="form-group">
                                        ...
                                        <small class="form-text text-muted">
                                            ...
                                            fill an existing SO number to update. Rows with the same blank header
                                            <code>project_code</code> + <code>stock_date</code> + <code>notes</code>
                                            are grouped into one document. <code>location</code> and
                                            <code>person_in_charge</code> are required.
                                            <a href="{{ route('supplies.stock-outs.template') }}">Download template</a>
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            $('#btn-export-stock-out').on('click', function(e) {
                e.preventDefault();
                const params = $.param({
                ...
                window.location.href = "{{ route('supplies.stock-outs.export') }}?" + params;
            });
        });
    </script>
@endsection
