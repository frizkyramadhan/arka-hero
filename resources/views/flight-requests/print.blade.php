                        ...
                        <div style="margin-bottom: 12px;">
                            ...
                            <table class="flight-table" style="margin-top: 0;">
                                ...
                                @else
                                    ...
                                    <tbody>
                                        @foreach ($flightRequest->followers as $key => $follower)
                                            @php
                                                ...
                                                $printDepartment =
                                                    $follower->department ??
                                                    ($follower->administration?->position?->department
                                                        ?->department_name ??
                                                        '-');
                                                $printProject = $follower->administration?->project;
                                                $printProjectLabel = $printProject
                                                    ? $printProject->project_name ?? ($follower->project ?? '-')
                                                    : $follower->project ?? '-';
                                            @endphp
                                            <tr>
                                                <td style="text-align: center;">{{ $key + 1 }}</td>
                                                <td>{{ $follower->displayName() }} / {{ $follower->nik ?? 'N/A' }}
                                                </td>
                                                <td>{{ $isEmployeeFollower ? $printPosition : '-' }}</td>
                                                <td>{{ $isEmployeeFollower ? $printProjectLabel : '-' }}</td>
                                                <td>{{ $isEmployeeFollower ? $printDepartment : '-' }}</td>
