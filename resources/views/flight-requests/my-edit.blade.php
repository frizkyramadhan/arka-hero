                        ...
                        @php
                            $editLotFollowerDetails =
                                ...
                                $flightRequest->officialTravel->details->isNotEmpty()
                                    ? $flightRequest->officialTravel->details
                                    : null;
                        @endphp
                        <div id="lot_followers_card" class="card card-primary card-outline elevation-3 mb-3"
                            style="{{ $editLotFollowerDetails ? '' : 'display: none;' }}">
                            <div class="card-header">
                                <h3 class="card-title">
                                    <i class="fas fa-users mr-2"></i>
                                    <strong>Followers</strong>
                                    <span class="badge badge-primary align-middle ml-2"
                                        id="lot_followers_count">{{ $editLotFollowerDetails ? $editLotFollowerDetails->count() : 0 }}</span>
                                </h3>
                            </div>
                            <div class="card-body p-0">
                                <div class="followers-list" id="lot_followers_list">
                                    @if ($editLotFollowerDetails)
                                        @foreach ($editLotFollowerDetails as $detail)
                                            @php $fw = $detail->follower; @endphp
                                            @if ($fw)
                                                <div class="follower-item">
                                                    <div class="follower-info">
                                                        <div class="follower-name">
                                                            {{ $fw->employee->fullname ?? 'Unknown Employee' }}
                                                        </div>
                                                        <div class="follower-position">
                                                            {{ $fw->position->position_name ?? 'No Position' }}
                                                        </div>
                                                        <div class="follower-meta">
                                                            <span class="follower-nik"><i class="fas fa-id-card"></i>
                                                                {{ $fw->nik }}</span>
                                                            <span class="follower-department"><i
                                                                    class="fas fa-sitemap"></i>
                                                                {{ $fw->position->department->department_name ?? 'No Department' }}</span>
                                                        </div>
                                                        <div class="follower-project">
                                                            <i class="fas fa-project-diagram"></i>
                                                            {{ $fw->project->project_code ?? 'No Code' }} :
                                                            {{ $fw->project->project_name ?? 'No Project' }}
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
...
@section('styles')
    <style>
        ...
        /* LOT followers (same as my-create) */
