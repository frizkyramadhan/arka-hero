...
@section('styles')
    ...
    <style>
        ...
        .fuel-scan-btn:hover { background: #e9ecef; border-color: #007bff; }
        .fuel-scan-btn i { font-size: 2.5rem; color: #007bff; }
        #confirm-panel, #manual-panel { display: none; }
        .fuel-sticky-actions {
            position: sticky;
            bottom: 0;
            background: #fff;
        ...
        #receipt-preview { max-height: 220px; object-fit: contain; width: 100%; border-radius: 8px; }
    </style>
@endsection
        ...
        <div class="container-fluid" style="max-width: 640px">
            ...
            <form id="fuel-form" action="{{ route('fuel-records.my-requests.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div id="confirm-panel" class="card card-success card-outline">
                    <div class="card-header"><h3 class="card-title">Confirm details</h3></div>
                    <div class="card-body">
                ...
                <div id="manual-panel" class="card card-secondary card-outline">
                    <div class="card-header"><h3 class="card-title">Manual entry</h3></div>
                    <div class="card-body">
                    ...
                    <div class="card-footer fuel-sticky-actions">
                        ...
                        <button type="button" class="btn btn-secondary btn-block" id="btn-back-scan-2">Back</button>
                    </div>
                </div>
            </form>
        </div>
...
@section('scripts')
    ...
    <script>
        (function() {
            ...
            var mode = 'scan'; // scan | confirm | manual
            ...
            function showPanel(name) {
                ...
                // Disable fields in hidden panels so only one set submits
            ...
            showPanel('scan');
            @if(old('vehicle_id') || $errors->any())
                @if(old('receipt_path'))
                    showPanel('confirm');
                    $('#receipt_path').val(@json(old('receipt_path')));
                @else
                    showPanel('manual');
                @endif
            @endif
