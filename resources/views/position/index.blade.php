                                ...
                                <div class="card-tools">
                                    <ul class="nav nav-pills ml-auto">
                                        <li class="nav-item mr-2">
                                            <a class="btn btn-info" href="{{ url('positions/export') }}"><i
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-10">
                                            <select name="department_id"
                                                class="form-control @error('department_id') is-invalid @enderror select2bs4">
                                                <option value="">- Select Department -</option>
                                                @foreach ($departments as $dept)
                                                    <option value="{{ $dept->id }}"
                                                        {{ old('department_id') == $dept->id ? 'selected' : null }}>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-10">
                                            <select name="position_status"
                                                class="form-control @error('position_status') is-invalid @enderror">
                                                <option value="1"
                                                    {{ old('position_status') == '1' ? 'selected' : '' }}>
                                                    Active</option>
                                                <option value="0"
                                                    {{ old('position_status') == '0' ? 'selected' : '' }}>Inactive
                                                </option>
                                            </select>
        ...
        <div class="modal fade" id="modal-import">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form class="form-horizontal" action="{{ url('positions/import') }}" method="POST"
    ...
    @if (session('failures'))
        <div class="modal fade" id="failureModal" tabindex="-1" role="dialog" aria-labelledby="failureModalLabel"
            ...
            <div class="modal-dialog modal-xl" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="failureModalLabel">Import Validation Errors</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                            ...
                            <table class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    @foreach (session('failures') as $failure)
                                        <tr>
                                            <td>{{ $failure['sheet'] ?? 'Unknown' }}</td>
                                            <td>{{ $failure['row'] ?? '-' }}</td>
                                            <td>{{ $failure['attribute'] ?? '-' }}</td>
                                            <td>{{ $failure['value'] ?? '-' }}</td>
                                            <td class="text-danger">{{ $failure['errors'] ?? '-' }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
