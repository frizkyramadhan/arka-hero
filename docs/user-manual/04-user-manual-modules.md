...
- **[Flight Management](#bab-14-flight-management)**
    ...
    - [3.1 Mengubah approver pending (Update Approvers)](#mengubah-approver-pending-frf)
...
| **LSL Usage Mode**                | Cara memakai hak **Long Service Leave** pada formulir **Cuti Panjang**: **Take Leave Only** (cuti saja), **Cash Out Only** (pencairan tunai tanpa cuti), atau **Combined** (keduanya). Menentukan bidang yang aktif dan angka **Total Days** yang mengurangi saldo LSL.                                                                                                                                                                                            |
...
7. Untuk pilihan **Cuti Panjang** — **Staff / Non Staff** (kode **4.01**), muncul kartu kuning **Long Service Leave (LSL)**. Pilih dahulu **LSL Usage Mode** lewat tiga tombol berlebar sama:

    - **Take Leave Only** — mengambil cuti tanpa pencairan tunai yang dihitung: isi **Leave Date** dan **Back to Work Date** (langkah 8); **Leave Days** otomatis mengikuti rentang tanggal saat Anda menekan **Apply** pada kalender (bisa disesuaikan manual); bidang **Cash Out** tetap terlihat **hanya baca** dan **tidak** masuk perhitungan **Total Days**; nilai cash out yang pernah disimpan **tidak dihapus** bila Anda mengganti mode.
    - **Cash Out Only** — hanya mencairkan hari LSL tanpa mengambil cuti: baris **Leave Date** / **Back to Work Date** **disembunyikan** pada form (nilai tanggal di form **tidak dihapus** saat berpindah mode); isi **Cash Out**; **Leave Days** hanya baca dan **tidak** dihitung; **Total Days** = hari cash out saja (tetap mengurangi saldo LSL).
    - **Combined** — cuti dan pencairan sekaligus: isi rentang tanggal seperti biasa, isi **Cash Out** bila ada hari yang dicairkan (**tanpa** checkbox); **Total Days** = **Leave Days** + **Cash Out** (contoh pada [Gambar 3.11](#leave-request-create-cutipanjang): rentang 01/07/2026–10/07/2026 → **8** hari cuti + **20** hari cash out = **28** hari total).

    **Total Days** pada kartu LSL adalah jumlah yang akan **mengurangi saldo LSL** sesuai mode aktif. Saat **mengubah pengajuan** (**Edit Request** / **My Edit**), mengganti **LSL Usage Mode** **tidak** mereset tanggal, **Leave Days**, atau **Cash Out** — sesuaikan manual bila perlu.

<p align="center" id="leave-request-create-cutipanjang">
    <img
        src="images/leave_request_create_cuti_panjang_lsl_modes.png"
        alt="Formulir Cuti Panjang Staff 4.01: Leave Type Cuti Panjang sisa 50 hari, Leave Period 13 Dec 2024–12 Dec 2029, Leave Date 01/07/2026–10/07/2026, kartu Long Service Leave dengan LSL Usage Mode Combined aktif, Leave Days 8 hari, Cash Out 20 hari, Total Days 28 hari"
        style="max-width: 90%; width: 90%; height: auto;"
    /><br/><em>Gambar 3.11 — Contoh <strong>Cuti Panjang - Staff</strong> (<strong>4.01</strong>) dengan mode <strong>Combined</strong>: rentang <strong>Leave Date</strong> menghasilkan <strong>Leave Days</strong> 8 hari, ditambah <strong>Cash Out</strong> 20 hari, sehingga <strong>Total Days</strong> 28 hari yang akan mengurangi saldo LSL (langkah 7).</em>
</p>

8. Isi **Leave Date** (satu hari atau rentang hari kerja) dan **Back to Work Date** lewat pemilih tanggal pada formulir — **wajib** untuk LSL mode **Take Leave Only** dan **Combined**. Untuk **Cash Out Only**, baris tanggal tidak ditampilkan pada form; pada halaman detail dan cetakan, **Start Date** / **End Date** (atau **Tanggal Mulai** / **Tanggal Selesai**) ditampilkan sebagai **—** karena tidak ada cuti yang diambil. Tentukan rentang yang selaras dengan sisa entitlement dan aturan penggunaan jenis cuti yang dipilih; **Total Days** pada form biasanya mengikuti rentang tersebut kecuali Anda menyesuaikan manual sesuai petunjuk di layar (pada LSL, angka pada kartu LSL yang mengatur pemotongan saldo). Perilaku kalender mengikuti jenis **project**: pada project **non-roster** (misalnya **HO**, **BO**, **APS**, **021C**, dan setara), **Sabtu** dan **Minggu** **tidak** dipilih; pada project **roster** (contoh **017C**, **022C**) semua tanggal dapat dipilih. **Hari libur nasional** dari master data ikut ditampilkan di kalender (sering dengan penanda atau tooltip) agar Anda tidak memilih tanggal yang bertabrakan dengan libur resmi. Setelah rentang benar, klik **Apply**; gunakan **Clear** bila perlu mengulang pemilih tanggal. Pada LSL, **Apply** juga menyelaraskan **Leave Days** dengan rentang tanggal bila mode Anda menghitung hari cuti.
...
— **Cuti Panjang / LSL / 4.01**: blok **LSL Breakdown** memisahkan **Leave Taken**, **Cash Out**, serta **Total LSL Used**. Bila pengajuan memakai mode **Cash Out Only**, **Start Date** dan **End Date** pada panel utama menampilkan **— (Cash out only, no leave taken)** dan **Back to Work Date** biasanya kosong. Catatan kuning dapat muncul bila ada hari **Cash Out**. Tombol **Print** membuka **Formulir Permohonan Cuti**: nomor register (**No. …**) berada **di bawah** judul **FORMULIR PERMOHONAN CUTI**, rata tengah; pada cetakan **Cash Out Only**, baris **Tanggal Mulai** dan **Tanggal Selesai** berisi **—** tanpa teks tambahan. **Flight Request** terkait (bila ada) biasanya ditampilkan di kolom sisipan aplikasi Anda.

<p align="center" id="leave-request-detail-additional-lsl">
    <img
        src="images/leave_request_detail_lsl_flight.png"
        alt="Leave Request Detail Cuti Panjang Staff breakdown Leave Taken Cash Out Total LSL Used Start End Date dash cash out only sidebar Flight Request Draft Selected Approvers Pending tombol Print"
        style="max-width: 85%; width: 85%; height: auto;"
    /><br/><em>Gambar 3.20 — <strong>Additional Information</strong> berupa <strong>LSL Breakdown</strong>, tanggal **—** bila hanya cash out, dan tautan blok <strong>Flight Request</strong> (contoh <strong>Cuti Panjang - Staff</strong> <strong>4.01</strong>).</em>
</p>
...
**Catatan:** Pengeditan **hanya** untuk status **Draft**. FRF **Submitted** tidak dapat diedit; gunakan **Update Approvers** untuk mengganti approver pending (lihat [bagian 3.1](#mengubah-approver-pending-frf)).
...
| FRF **Submitted** tidak bisa **Edit**  | FRF sudah diajukan ke approver                                                                       | Isian FRF tidak bisa **Edit** lagi; HR mengganti approver pending lewat **Update Approvers** di kartu **Approval Status** (lihat [bagian 3.1](#mengubah-approver-pending-frf)). |
...
5. **Isi formulir pengajuan**
    ...
    - **Leave Type** — **Cuti Tahunan** / **Cuti Panjang** menampilkan **Leave Period** dan membatasi **Leave Date** ke periode itu. **Izin Dibayar** / **Izin Tanpa Upah** menyembunyikan field **Leave Period** di form; tanggal tidak dibatasi periode, tetapi sisa hari periode berjalan tetap berlaku. Bila tipe tidak bisa dipilih, hubungi HR soal saldo cuti / **Leave Entitlement**.
    - **Pola per jenis cuti** — **Cuti Tahunan (1.01)** tanpa dokumen tambahan wajib; **Izin Dibayar (2.xx)** dapat memunculkan **Supporting Document**; **Izin Tanpa Upah (3.01)** wajib isi **Reason**; **Cuti Panjang (4.01)** memunculkan kartu **LSL** dengan **LSL Usage Mode** (**Take Leave Only**, **Cash Out Only**, **Combined**), **Leave Days**, **Cash Out**, dan **Total Days**.
...
Gunakan daftar ini untuk **memantau status** yang sama untuk semua jalur (**Draft**, **Submitted**, **Approved**, **Issued**, dll.) dan membuka detail **View** / **Edit** bila tombol tersebut tersedia. Tombol **Edit** pada kolom **Actions** **hanya** tampil untuk status **Draft** — FRF **Submitted** tidak memiliki **Edit** di daftar maupun di detail. Kolom **Request Type** memuat badge seperti **Leave Based**, **Travel Based**, dan **Standalone**. Contoh halaman detail setelah **View**: **Draft** pada [Gambar 6.1b](#my-flight-request-detail-draft); **Submitted** dengan kartu **Approval Status** pada [Gambar 6.1c](#my-flight-request-detail-submitted). Alur persetujuan cuti atau LOT tetap berjalan di modul masing-masing; permintaan tiket mengikuti kebijakan **Flight Management** dan koordinasi **HR HO Balikpapan**.
